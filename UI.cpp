...
bool ImGui::ToggleButton(const char* label, bool* v, const ImVec2& size_arg)
{
	...
	ImVec2 size = ImGui::CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 5.0f, label_size.y + style.FramePadding.y * 5.0f);
	...
	if (*v)
		PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_MenuMain));
	...
	if (*v)
		PopStyleColor();
	if (*v)
		window->DrawList->AddRect(bb.Min + ImVec2(3.5f, 3), bb.Max + ImVec2(-2.5f, -3.f), GetColorU32( ImGuiCol_MenuMain ), 0, 15, 1);

	window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 1.f)), 0, 15, 1);
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)));
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 1.f)), 0, 15, 0.7f);
	...
	ImGui::RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, std::string("Key: " + std::string(buf_display)).c_str(), NULL, NULL, style.ButtonTextAlign, &clip_rect);
